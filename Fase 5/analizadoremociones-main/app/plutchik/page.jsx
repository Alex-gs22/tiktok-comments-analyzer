...
import { emotionDistribution } from '../../src/lib/mockData';
import { Combine, Info } from 'lucide-react';

const wheelData = {};
emotionDistribution.forEach((d) => {
  wheelData[d.emotion] = { porcentaje: d.porcentaje, total: d.total };
});
                  ...
                  return (
                    <div key={emotion} className="flex items-center gap-3 p-2.5 rounded-xl bg-[rgba(255,255,255,0.02)]">
                      ...
                      <span className="text-[10px] text-[#6b6b80]">
                        {map?.merged ? `← ${map.original.join(' + ')}` : '← Original Plutchik'}
                      </span>
                      <span className="ml-auto text-xs font-bold" style={{ color: config.color }}>
                        {wheelData[emotion]?.porcentaje || 0}%
                      </span>
                    </div>
                  );
                })}
